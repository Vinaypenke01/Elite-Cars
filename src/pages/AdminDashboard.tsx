...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        if (settings) {
            setSettingsForm({
                ...
                mon_sat: settings.businessHours?.mon_sat || '',
    ...
    const stats = [
        ...
        { label: 'Active Bookings', value: bookings.filter(b => b.status === 'pending').length, icon: Calendar, color: 'text-amber-500' },
        { label: 'Total Bookings', value: bookings.length, icon: TrendingUp, color: 'text-green-500' },
        { label: 'Admins', value: 1, icon: Users, color: 'text-purple-500' },
    ];
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">
                            Welcome back, {adminProfile?.displayName || 'Administrator'}
                        </p>
                    </div>
                                    ...
                                    <TableBody>
                                        {bookings.map((booking) => (
                                            <TableRow key={booking.id}>
                                                <TableCell>
                                                    <div className="font-medium text-blue-100">{booking.customerName}</div>
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between">
                                <div>
                                    ...
                                    <CardDescription>Add, update or remove vehicles from the public display.</CardDescription>
                                </div>
